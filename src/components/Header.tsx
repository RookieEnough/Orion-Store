...
export const Header = memo(function Header() {
  ...
  const storeUpdateAvailable =
    config?.latestStoreVersion &&
    config.latestStoreVersion !== CURRENT_STORE_VERSION;

  const handleUpdateStore = () => {
    if (config?.storeDownloadUrl) {
      window.location.href = config.storeDownloadUrl;
    }
  };
    ...
    <header className="absolute top-0 left-0 w-full z-20 px-6 py-6 flex justify-between items-center">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <i
            className={`fas ${
              theme === 'light' ? 'fa-sun' : theme === 'dusk' ? 'fa-cloud-sun' : 'fa-moon'
            }`}
          />
        </button>
      </div>

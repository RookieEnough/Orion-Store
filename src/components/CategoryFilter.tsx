...
export const CategoryFilter = memo(function CategoryFilter({ platform }: CategoryFilterProps) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (!(e.target as HTMLElement).closest('#category-dropdown')) setIsOpen(false);
    };
  ...
  return (
    <div className="relative z-10 mb-6 animate-fade-in flex items-center gap-3">
          ...
          <ul className="absolute mt-2 w-full bg-card/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-theme-border max-h-60 overflow-y-auto z-50 animate-fade-in no-scrollbar">
            {categories.map((category) => (
              <li
                ...
                className={`px-4 py-3 cursor-pointer hover:bg-primary/20 transition-colors ${selectedCategory === category ? 'font-bold text-primary' : 'text-theme-text'}`}
              >
                {category}
              </li>
            ))}
      ...
      <button
        onClick={refresh}
        className={`shrink-0 w-12 h-12 rounded-2xl border border-theme-border bg-card flex items-center justify-center text-theme-sub hover:text-primary hover:border-primary transition-all shadow-xs active:scale-95 ${isRefreshing ? 'animate-spin text-primary' : ''}`}
        title="Refresh Data"
      >

...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const { isUnlocked: isDevUnlocked, handleTap: handleDevTap, toastMessage: devToastMessage, showToast: showDevToast } = useDevMode();
  ...
  const { apps, isLoading, isRefreshing, config, installedVersions, refresh, registerInstall } = useApps(
    useRemoteJson,
    githubToken
  );
  ...
  const handleDownload = useCallback(
    (app: AppItem, specificUrl?: string) => {
  ...
  const checkHasUpdate = useCallback(
    (app: AppItem) => hasUpdate(installedVersions[app.id], app.latestVersion),
    [installedVersions]
  );
